...
#include "hal/hal_metrics.h"    // system usage + watchdog heartbeat
#include <iostream>
#include <vector>
#include <string>
#include <thread>
...
// ── Wi-Fi task — runs every frame, drives capture to handshake logger +
// KARMA responder, and drains scan results into the wardriving queue.
static void wifi_task() {
    hal_wifi_tick();
}

// ── Power task — runs every second ───────────────────────────────────────
// Also drives the at-rest loot encryption pump (mailboxes the heavy KDF/
// AES work so it never blocks the UI). The passphrase for the pump is
// configured at boot via hal_crypto_init().
static void power_task() {
    hal_power_tick();
#ifdef VOIDOS_RPI5
    // Encrypt any freshly-written loot payloads at most once per 5 s.
...
void setup() {
...
#ifdef VOIDOS_RPI5
    hal_wifi_init();       // nl80211 transport, capability + interface probe
#endif
    ...
    hal_web_ctrl_init();
    hal_metrics_init();                      // scorer + watchdog (1 s tick)
    sched_add("input",  input_task, 0,    1);  // every frame, highest priority
    sched_add("app",    app_task,    0,    2);  // active app logic
    sched_add("wifi",   wifi_task, 0,    3);   // radio capture / scan drain
